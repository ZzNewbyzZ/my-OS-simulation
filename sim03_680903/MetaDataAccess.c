...
Algorithm: opens file, acquires opcode data,
...
int getOpCodes(char *fileName, OpCodeType **opCodeDataHead)
{
	...
	*opCodeDataHead = NULL;
	...
	if(accessResult == LAST_OPCMD_FOUND_MSG)
	{
		...
		// otherwise, assume start count not equal to end cound
		else
		{		
...
Postcondition: assigns new structure node to beginning of linked list
			   or an of established linked listunchanged
Exceptions: none
...
Postcondition: if test value is a value '0' < '9',
...
Algorithm: uses enum/constant values as indices to select display string,
		   then displays error message iwith selected string
